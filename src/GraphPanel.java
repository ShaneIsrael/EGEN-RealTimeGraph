...
public class GraphPanel extends JPanel {
	...
	private void drawGraph(Graphics2D graph2D) {
		// Drag graph
		...
		drawGraphPoints(points);

		for (int i = 0; i < tests.size(); i++) {
			graph2D.setColor(tests.get(i).getColor());
			drawGraphPoints(tests.get(i).getGraph());
		}
		graph2D.dispose();
	}

	private void drawGraphPoints(ArrayList<Point> p) {
		graph2D.setStroke(new BasicStroke(3f));
		Point firstP;
		if (p.size() > 1) {
			firstP = p.get(0);
